...
app.post('/api/admin/login', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!loginIdentifier || !password) {
      return res.status(400).json({ success: false, error: 'Email aur password dono zaroori hain' });
    }

    const adminUser = await getAdminByLoginIdentifier(loginIdentifier);
    ...
    if (!adminUser) {
      ...
      // Return simple generic message per spec
    ...
    if (adminUser.isActive === false) {
      ...
      return res.status(403).json({
        success: false,
        error: 'Account disabled hai. Kripya Super Admin se sampark karein.',
      });
    }
    ...
    if (adminUserAny.lockedUntil && new Date() < new Date(adminUserAny.lockedUntil)) {
      const waitMin = Math.max(
        1,
        Math.ceil((new Date(adminUserAny.lockedUntil).getTime() - Date.now()) / (1000 * 60))
      );
      return res.status(403).json({
        success: false,
        error: `Account 15 minute ke liye lock ho gaya hai (5 failed attempts). Kripya ${waitMin} minute baad koshish karein.`,
      });
    }
    ...
    if (!isMatch) {
      ...
      try {
        await db
          .update(admins)
          .set({ failedAttempts: newAttempts, lockedUntil: lockDate })
      ...
      await logActivity(
        ...
        'admin',
        String(adminUser.id),
        { attempts: newAttempts, locked: newAttempts >= 5 },
        clientIp,
        adminUser.email || adminUser.adminId
      );
      ...
      if (newAttempts >= 5) {
        return res.status(403).json({
          success: false,
          error: 'Account 15 minute ke liye lock ho gaya hai (5 failed attempts). Kripya 15 minute baad koshish karein.',
        });
      }

      // Return simple generic message per spec without revealing which is wrong
...
app.get(
  ...
  async (req: AdminAuthRequest, res: Response) => {
    try {
      ...
      res.json({ success: true, admin: foundAdmin[0] });
    } catch (err: unknown) {
...
app.post(
  ...
  async (req: AdminAuthRequest, res: Response) => {
    try {
      ...
      res.json({ success: true, banner: inserted[0] });
    } catch (err: unknown) {
  ...
  async (_req: AdminAuthRequest, res: Response) => {
    try {
      ...
      const mapped = userList.map((u) => {
        const uOrders = orderList.filter((o) => o.userId === u.id || o.customerPhone === u.phone);
        ...
        return {
          ...
          totalSpent,
          lastOrderDate: uOrders.length ? uOrders[0].createdAt : null,
        };
      });
...
app.get(
  ...
  async (_req: AdminAuthRequest, res: Response) => {
    try {
      ...
      res.status(500).json({ success: false, error: 'Failed to load staff accounts' });
    }
  }
);
